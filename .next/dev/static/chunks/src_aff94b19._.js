...
function TodayGamesPage({ initialData }) {
    ...
    const filteredGames = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useMemo"])({
        "TodayGamesPage.useMemo[filteredGames]": ()=>{
            const filtered = initialData.games.filter({
                "TodayGamesPage.useMemo[filteredGames].filtered": (game)=>game.date?.slice(0, 10) === selectedDate
            }["TodayGamesPage.useMemo[filteredGames].filtered"]).filter({
                "TodayGamesPage.useMemo[filteredGames].filtered": (game)=>teamQuery ? `${game.homeTeam} ${game.awayTeam}`.toLowerCase().includes(teamQuery.toLowerCase()) : true
            }["TodayGamesPage.useMemo[filteredGames].filtered"]).filter({
                "TodayGamesPage.useMemo[filteredGames].filtered": (game)=>game.hotBatScore >= minHotBatScore
            }["TodayGamesPage.useMemo[filteredGames].filtered"]).filter({
                "TodayGamesPage.useMemo[filteredGames].filtered": (game)=>game.parkHrFactor >= minParkHrFactor
            }["TodayGamesPage.useMemo[filteredGames].filtered"]);
            return filtered.length > 0 ? filtered : initialData.games;
        }
    }["TodayGamesPage.useMemo[filteredGames]"], [
        initialData.games,
        minHotBatScore,
...
_s(TodayGamesPage, "gdtRSSM/WQ4WLb5Jub/EtEoCkyk=", false, function() {
    return [
...
//# sourceMappingURL=src_aff94b19._.js.map